...
<script type="text/javascript">
	...
	{						
			if( !$("#datetimepicker1").val() ) {         				
				$(".alert").removeClass("in").show();
				$(".alert").html('<button type="button" class="close" data-dismiss="alert">×</button> <strong>¡Verifique!</strong> Debe ingresar fecha inicial.');
				$(".alert").delay(1200).addClass("in").fadeOut(2000);
	         	return false;
	    	}
	    	if( !$("#datetimepicker2").val() ) {         	
	    		$(".alert").removeClass("in").show();
	    		$(".alert").html('<button type="button" class="close" data-dismiss="alert">×</button> <strong>¡Verifique!</strong> Debe ingresar fecha final.');
				$(".alert").delay(1200).addClass("in").fadeOut(2000);
	         	return false;
	    	}    	
	    	var fechaInicial = $("#datetimepicker1").val();    	
			var fechaFinal = $("#datetimepicker2").val();
			if(Date.parse(fechaInicial)>Date.parse(fechaFinal))
			{							
				$(".alert").removeClass("in").show();			
				$(".alert").html('<button type="button" class="close" data-dismiss="alert">×</button> <strong>¡Verifique!</strong> La fecha de finalización debe ser posterior a la fecha de inicio.');
				$(".alert").delay(1800).addClass("in").fadeOut(2000);
				return false;
			}
		return true;
	}
